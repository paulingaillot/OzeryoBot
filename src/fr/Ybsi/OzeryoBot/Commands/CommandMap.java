...
import fr.Ybsi.OzeryoBot.Commands.Default.BlackList;
import fr.Ybsi.OzeryoBot.Commands.Default.ChannelSet;
import fr.Ybsi.OzeryoBot.Commands.Default.CommandDefaut;
import fr.Ybsi.OzeryoBot.Commands.Default.Cosmetics;
import fr.Ybsi.OzeryoBot.Commands.Default.EmojiCommand;
import fr.Ybsi.OzeryoBot.Commands.Default.Fortnite;
import fr.Ybsi.OzeryoBot.Commands.Default.Google;
import fr.Ybsi.OzeryoBot.Commands.Default.Hypixel;
import fr.Ybsi.OzeryoBot.Commands.Default.NSFW;
import fr.Ybsi.OzeryoBot.Commands.Default.Pub;
import fr.Ybsi.OzeryoBot.Commands.Default.WOT;
import fr.Ybsi.OzeryoBot.Commands.Default.reputation;
import fr.Ybsi.OzeryoBot.Commands.Game.Achivement;
import fr.Ybsi.OzeryoBot.Commands.Game.Attack;
import fr.Ybsi.OzeryoBot.Commands.Game.Build;
import fr.Ybsi.OzeryoBot.Commands.Game.City;
import fr.Ybsi.OzeryoBot.Commands.Game.Competence;
import fr.Ybsi.OzeryoBot.Commands.Game.Daily;
import fr.Ybsi.OzeryoBot.Commands.Game.Factory;
import fr.Ybsi.OzeryoBot.Commands.Game.Habitations;
import fr.Ybsi.OzeryoBot.Commands.Game.Heroe;
import fr.Ybsi.OzeryoBot.Commands.Game.LootBox;
import fr.Ybsi.OzeryoBot.Commands.Game.Mana;
import fr.Ybsi.OzeryoBot.Commands.Game.Pays;
import fr.Ybsi.OzeryoBot.Commands.Game.Pray;
import fr.Ybsi.OzeryoBot.Commands.Game.Quests;
import fr.Ybsi.OzeryoBot.Commands.Game.Shop;
import fr.Ybsi.OzeryoBot.Commands.Game.Soldier;
import fr.Ybsi.OzeryoBot.Commands.Game.Spy;
import fr.Ybsi.OzeryoBot.Commands.Game.Trade;
import fr.Ybsi.OzeryoBot.Commands.Game.Zoo;
import fr.Ybsi.OzeryoBot.Commands.Game.hourly;
import fr.Ybsi.OzeryoBot.Commands.HelpCommand;
import fr.Ybsi.OzeryoBot.Commands.Mail;
...
import fr.Ybsi.OzeryoBot.Utils.CoolDown;
...
public final class CommandMap {
    ...
    public CommandMap(DiscordBot DiscordBot2) {
        ...
        this.registerCommands(new CommandDefaut(DiscordBot2), new HelpCommand(this), new MusicCommands(), new Google(), new Hypixel(), new ChannelSet(), new NSFW(), new City(), new hourly(), new Daily(), new Build(), new reputation(), new Attack(), new EmojiCommand(), new WOT(), new Mana(), new Fortnite(), new Pays(), new Soldier(), new Shop(), new Zoo(), new Habitations(), new Trade(), new Factory(), new BlackList(), new Quests(), new Pray(), new Spy(), new Achivement(), new Cosmetics(), new Heroe(), new LootBox(), new Mail(), new Competence(), new Pub());
    }
    ...
    public void registerCommand(Object object) {
        for (Method method : object.getClass().getDeclaredMethods()) {
            ...
            SimpleCommand simpleCommand = new SimpleCommand(command2.name(), command2.abbrev(), command2.topic(), command2.descfr(), command2.descen(), command2.type(), object, method, command2.power(), command2.use());
    ...
    public void commandConsole(String command2) {
        ...
        try {
            CommandMap.execute((SimpleCommand)object[0], command2, (String[])object[1], null, null, null, null);
        }
        catch (Exception e) {
            System.out.println("La methode " + ((SimpleCommand)object[0]).getMethod().getName() + " n'est pas correctement initialis\ufffd." + e.getMessage());
            e.printStackTrace();
        }
    }
     ...
     */
    public static boolean commandUser(User user, String command2, Message message, MessageChannel channel, Guild guild) {
        ...
        try {
            ...
            if (((SimpleCommand)object[0]).getTopic() == command.Topics.Game) {
                ...
                if (dif > 0L && !user.getId().equals("431460839942389760")) {
                    ...
                    channel.sendMessage("OzeryoBot imporve itself to version 3.0 in " + mDay + " days, " + mHour + " hours and " + mMinute + " minutes. Pre-register to be notified of the release of the 3rd version of OzeryoBot and get 1 month of free premium now thanks to the ``=register`` command. There is actually " + data.getRegisters() + "  people registered.").queue();
                    return true;
                }
                if (!game) {
                ...
                if (tuto == 2 && ((SimpleCommand)object[0]).getMethod().getName() != "hourly" && ((SimpleCommand)object[0]).getMethod().getName() != "hr") {
                    if (lang == command.Language.fr) {
                ...
                if (tuto == 3 && ((SimpleCommand)object[0]).getMethod().getName() != "city" && ((SimpleCommand)object[0]).getMethod().getName() != "c") {
                    if (lang == command.Language.fr) {
                ...
                if (Event.Summer()) {
                    do {
                        ...
                        if (lastpallier % 10 == 5) {
                            ...
                            if (LootBox.test(user) <= (Integer)building.get("armurerie") * 5 + 20) {
                                ...
                                data.getProfils().get(user.getId()).setWeapons(weapons);
                                if (lang == command.Language.fr) {
                                    channel.sendMessage("Vous venez de franchir le pallier " + lastpallier + ". Vous avez donc obtenu une arme : " + weapon + " de niveau " + aleaWeapon2 + ".").queue();
                                }
                                if (lang == command.Language.en) {
                                    channel.sendMessage("You just croos the step " + lastpallier + ".So you won a weapon : " + weapon + " level " + aleaWeapon2 + ".").queue();
                                }
                            } else {
                                if (lang == command.Language.fr) {
                                    channel.sendMessage("Vous venez de franchir le pallier " + lastpallier + ". Vous n'avez pas pu obtenir d'arme car votre entrepot est plein.").queue();
                                }
                                if (lang == command.Language.en) {
                                    channel.sendMessage("You jsut cross the step " + lastpallier + ". But you didn't win any weapon because your repository is full.").queue();
                                }
                            }
                        }
                        if (lastpallier % 10 == 1) {
                            ...
                            if (LootBox.test(user) <= (Integer)building.get("armurerie") * 5 + 20) {
                                ...
                                data.getProfils().get(user.getId()).setArmor(armors);
                                if (lang == command.Language.fr) {
                                    channel.sendMessage("Vous venez de franchir le pallier " + lastpallier + ". Vous avez donc obtenu une armure : " + armor + " de niveau " + aleaArmor2 + ".").queue();
                                }
                                if (lang == command.Language.en) {
                                    channel.sendMessage("You just cross the step " + lastpallier + ". So you won an armor : " + armor + " level " + aleaArmor2 + ".").queue();
                                }
                            } else {
                                if (lang == command.Language.fr) {
                                    channel.sendMessage("Vous venez de franchir le pallier " + lastpallier + ". Vous n'avez pas pu obtenir d'arme car votre entrepot est plein.").queue();
                                ...
                                if (lang == command.Language.en) {
                                    channel.sendMessage("You jsut cross the step " + lastpallier + ". But you didn't win any armor because your repository is full.").queue();
                                }
                            }
                        }
                        if (lastpallier % 10 == 3) {
                        ...
                        if (lastpallier % 10 == 2) {
                            ...
                            if (lang == command.Language.fr) {
                                channel.sendMessage("Vous venez de franchir le pallier " + lastpallier + ". Vous obtenez donc +10% de bonus. Vous avez un bonus d'xp (d'event) de +" + bonus + "%.").queue();
                            }
                            if (lang == command.Language.en) {
                                channel.sendMessage("You just cross the step " + lastpallier + ". So you won +10% xp bonus. You have an xp bonus (for the event) of +" + bonus + "%.").queue();
                            }
                        }
                        if (lastpallier % 10 == 4) {
                            int level;
                        ...
                        if (lastpallier % 10 == 7) {
                            ...
                            if (lastpallier == 7) {
                                ...
                                if ((Integer)building.get("cirque") >= pets) {
                                    ...
                                    if (lang == command.Language.fr) {
                                        channel.sendMessage("Vous venez de franchir le pallier " + lastpallier + ". Vous avez donc obtenu le pet Aigle.").queue();
                                    }
                                    if (lang == command.Language.en) {
                                        channel.sendMessage("You just cross the step " + lastpallier + ". So you won the pet Eagle.").queue();
                                    }
                                }
                                ...
                                if (lang == command.Language.fr) {
                                    channel.sendMessage("Vous venez de franchir le pallier " + lastpallier + ". Vous avez donc obtenu la Plage offrant un bonus de 200.000 habitants.").queue();
                                }
                                if (lang == command.Language.en) {
                                ...
                                if (lang == command.Language.en) {
                                    channel.sendMessage("You just cross the step " + lastpallier + ". So you won the rank \"\ud83c\udfc4 Summer 2019\" on the Ozeryo official server.").queue();
                                }
                                ...
                                if ((Integer)building.get("cirque") >= pets) {
                                    ...
                                    data.getProfils().get(user.getId()).setPet(pet);
                                    if (lang == command.Language.fr) {
                                        channel.sendMessage("Vous venez de franchir le pallier " + lastpallier + ". Vous avez donc obtenu le pet Lion.").queue();
                                    }
                                    if (lang == command.Language.en) {
                                        channel.sendMessage("You just cross the step " + lastpallier + ". So you won the pet Lion.").queue();
                                    }
                                }
                                ...
                                data.getProfils().get(user.getId()).setHouses(houses);
                                if (lang == command.Language.fr) {
                                    channel.sendMessage("Vous venez de franchir le pallier " + lastpallier + ". Vous avez donc obtenu l'Ile paradisiaque offrant un bonus de 500k habitants.").queue();
                                }
                                if (lang == command.Language.en) {
                                    channel.sendMessage("You just cross the step " + lastpallier + ". So you won the paradise island which give a bonus of 500k people.").queue();
                                }
                                ...
                                if (Premium.Premium(User_Premium)) {
                                    ...
                                    if (lang == command.Language.en) {
                                        channel.sendMessage(String.valueOf(User_Premium.getName()) + " receive one Premium month more.").queue();
                                    }
                                } else {
                                    ...
                                    if (lang == command.Language.en) {
                                        channel.sendMessage("You just cross the step " + lastpallier + ". So you won one Premium month.").queue();
                                    }
                                }
                                ...
                                if ((Integer)building.get("cirque") >= pets) {
                                    ...
                                    data.getProfils().get(user.getId()).setPet(pet);
                                    if (lang == command.Language.fr) {
                                        channel.sendMessage("Vous venez de franchir le pallier " + lastpallier + ". Vous avez donc obtenu le pet Lievre.").queue();
                                    }
                                    if (lang == command.Language.en) {
                                        channel.sendMessage("You just cross the step " + lastpallier + ". So you won the pet Hare.").queue();
                                    }
                                }
                                ...
                                data.getProfils().get(user.getId()).setHouses(houses);
                                if (lang == command.Language.fr) {
                                    channel.sendMessage("Vous venez de franchir le pallier " + lastpallier + ". Vous avez donc obtenu la Piscine offrant un bonus de 1M habitants.").queue();
                                }
                                if (lang == command.Language.en) {
                                    channel.sendMessage("You just cross the step " + lastpallier + ". So you won the Swimming Pool wich give a bonus of 1M people.").queue();
                                }
                        ...
                        if (lastpallier % 10 == 8) {
                            ...
                            if (lang == command.Language.fr) {
                                channel.sendMessage("Vous venez de franchir le pallier " + lastpallier + ". Vous obtenez donc 15 jetons").queue();
                            }
                            if (lang == command.Language.en) {
                                channel.sendMessage("You just cross the stepr " + lastpallier + ". So you won 15 tokens").queue();
                            }
                        }
                        if (lastpallier % 10 == 9) {
                            if (lang == command.Language.fr) {
                        ...
                        if (lastpallier % 10 == 0) {
                            ...
                            if (lang == command.Language.fr) {
                                channel.sendMessage("Vous venez de franchir le pallier " + lastpallier + ". Vous obtenez donc " + nblb + " lootbox.").queue();
                            }
                            if (lang == command.Language.en) {
                                channel.sendMessage("You just cross the step " + lastpallier + ". So you won " + nblb + " lootbox.").queue();
                            }
                        }
                        ++lastpallier;
                    } while (true);
    ...
    public static void PublicLog(String message, JDA jda) {
        Guild ozeryo = jda.getGuildById("326345972739473410");
        TextChannel logChannel = ozeryo.getTextChannelById("540170055711719424");
    ...
    private static void execute(final SimpleCommand simpleCommand, final String command2, final String[] args, final Message message, final MessageChannel channel, final Guild guild, final User user) throws Exception {
        Runnable Uptime_Update = new Runnable(){
            ...
            public void run() {
                ...
                for (int i = 0; i < parameters.length; ++i) {
                    ...
                    if (parameters[i].getType() != MessageChannel.class) continue;
