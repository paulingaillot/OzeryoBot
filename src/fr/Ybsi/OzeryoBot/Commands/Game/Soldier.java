...
public class Soldier {
    ...
    private void soldier(MessageChannel channel, User user, Guild guild, String[] args, command.Language lang) {
        ...
        if (c1.equals("train")) {
            ...
            if (max) {
                for (c2 = 0; (long)price2 < money - 10L && (long)c2 < soldiers && (double)price_hab < (double)habitant * 0.25 || tuto == 8 && c2 < 100; ++c2) {
                    price = price2;
                    price_hab = c2;
            ...
            if ((eglise = TextFileWriter.read("/home/DiscordBot/Rasberry/données/Users/" + user.getId() + "/eglise.txt")).equals("Ares")) {
                ...
                if (lang == command.Language.en) {
                    message = String.valueOf(message) + "You obtain a bonus of " + soldier2 + " soldiers more thank to Ares.";
                }
            }
            ...
            if (lang == command.Language.fr) {
                channel.sendMessage("Vous venez de creer " + c2 + "soldats pour " + price + "$ et " + price_hab + " habitants." + message + " Vous pouvez encore former " + (soldiers - (long)c2) + " soldats actuelement.").queue();
            }
            if (lang == command.Language.en) {
                channel.sendMessage("You just train " + c2 + " soldiers for " + price + "$ and " + price_hab + " people." + message + " You can train " + (soldiers - (long)c2) + " soldiers more actually.").queue();
            }
            if (tuto == 8) {
