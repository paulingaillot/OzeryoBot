...
public class Habitations {
    ...
    private void houses(Message message, Guild guild, String[] args, User user, MessageChannel channel, String arg, ProfilData data, command.Language lang) {
        String c1;
        try {
            c1 = args[0];
        }
                ...
                try {
                    for (ArrayList<String> pet : animaux.values()) {
                        ...
                        if (lang == command.Language.fr) {
                            builder.addField(String.valueOf(pet.get(0)), "Bonus : " + message1 + "% d'habitants en plus", true);
                        }
                    ...
                    if (lang != command.Language.en) break block15;
                    builder.addField("Error", "You don't have any special houses", true);
                }
            }
            if (i == 0) {
                if (lang == command.Language.fr) {
                    builder.addField("Vide", "Vous n'avez actuelement aucunes maisons, commencer a en acheter des a present avec la commande ``=bid``.", true);
                }
                if (lang == command.Language.en) {
                    builder.addField("Anything", "You don't have any houses, you can win houses by using the command ``=bid``.", true);
                }
            } else {
                if (lang == command.Language.fr) {
