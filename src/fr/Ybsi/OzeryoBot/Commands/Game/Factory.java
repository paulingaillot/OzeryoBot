...
public class Factory {
    ...
    private void usine(Message message, Guild guild, String[] args, User user, MessageChannel channel, String arg, ProfilData data, command.Language lang) {
        String c1;
        String c2;
        ...
        if (level == 0) {
            if (lang == command.Language.fr) {
                channel.sendMessage("Vous devez construire votre laboratoire pour pouvoir acceder a cette commande.").queue();
            }
        ...
        if (c1.equals("start")) {
            if (etat.equals("broken")) {
                if (lang == command.Language.fr) {
                    channel.sendMessage("Vous ne pouvez pas demarrer votre laboratoire car celui ci est detruit").queue();
                }
                if (lang == command.Language.en) {
