    ...
    private void wot(User user, MessageChannel channel, String[] args, Guild guild, JDA jda, command.Language lang) {
        try {
            ...
            try {
                ...
                url = new URL(
                        "https://api.worldoftanks.eu/wot/account/info/?application_id=" + key + "&account_id=" + id);
                con = (HttpURLConnection) url.openConnection();
                con.setRequestMethod("GET");
                con.setRequestProperty("User-Agent", "Java client");
                con.setRequestProperty("Content-Type", "application/json");
            ...
            if (lang == command.Language.fr) {
                builder1.addField(
                        "\ud83d\udcaf EXP par partie", jsonObject.getAsJsonObject("data").getAsJsonObject(id)
                                .getAsJsonObject("statistics").getAsJsonObject("all").get("battle_avg_xp").toString(),
                        true);
            }
            if (lang == command.Language.en) {
                builder1.addField(
                        "\ud83d\udcaf Xp per game", jsonObject.getAsJsonObject("data").getAsJsonObject(id)
                                .getAsJsonObject("statistics").getAsJsonObject("all").get("battle_avg_xp").toString(),
                        true);
            }
            if (lang == command.Language.fr) {
                builder1.addField("\ud83d\udca5 Degat par partie",
                        jsonObject.getAsJsonObject("data").getAsJsonObject(id).getAsJsonObject("statistics")
                                .getAsJsonObject("all").get("avg_damage_assisted").toString(),
                        true);
            }
            if (lang == command.Language.en) {
                builder1.addField("\ud83d\udca5 Damage per game", jsonObject.getAsJsonObject("data").getAsJsonObject(id)
                        .getAsJsonObject("statistics").getAsJsonObject("all").get("avg_damage_assisted").toString(),
                        true);
            }
            if (lang == command.Language.fr) {
                builder1.addField("\ud83d\udca5 Degat moyen bloqués par partie",
                        jsonObject.getAsJsonObject("data").getAsJsonObject(id).getAsJsonObject("statistics")
                                .getAsJsonObject("all").get("avg_damage_blocked").toString(),
                        true);
            }
            if (lang == command.Language.en) {
                builder1.addField("\ud83d\udca5 Average damage blocked per game",
                        jsonObject.getAsJsonObject("data").getAsJsonObject(id).getAsJsonObject("statistics")
                                .getAsJsonObject("all").get("avg_damage_blocked").toString(),
                        true);
            }
            if (lang == command.Language.fr) {
                builder1.addField("\ud83c\udfae Parties joués", jsonObject.getAsJsonObject("data").getAsJsonObject(id)
                        .getAsJsonObject("statistics").getAsJsonObject("all").get("battles").toString(), true);
            }
            if (lang == command.Language.en) {
                builder1.addField("\ud83c\udfae Games played", jsonObject.getAsJsonObject("data").getAsJsonObject(id)
                        .getAsJsonObject("statistics").getAsJsonObject("all").get("battles").toString(), true);
            }
            if (lang == command.Language.fr) {
            ...
            if (lang == command.Language.fr) {
                builder1.addField("\ud83c\udfc6 Parties gagnés", jsonObject.getAsJsonObject("data").getAsJsonObject(id)
                        .getAsJsonObject("statistics").getAsJsonObject("all").get("wins").toString(), true);
            }
            if (lang == command.Language.en) {
                builder1.addField("\ud83c\udfc6 Games win", jsonObject.getAsJsonObject("data").getAsJsonObject(id)
                        .getAsJsonObject("statistics").getAsJsonObject("all").get("wins").toString(), true);
            }
            Double win = Double.parseDouble(jsonObject.getAsJsonObject("data").getAsJsonObject(id)
                    .getAsJsonObject("statistics").getAsJsonObject("all").get("wins").toString());
            Double losses = Double.parseDouble(jsonObject.getAsJsonObject("data").getAsJsonObject(id)
                    .getAsJsonObject("statistics").getAsJsonObject("all").get("losses").toString());
            Double ratio = win / losses;
            ratio = Utils.arrondi(2, ratio);
            builder1.addField("\ud83d\udd30 Ratio Wins/Losses", ratio + "%", true);
            if (lang == command.Language.fr) {
                builder1.addField("\ud83d\udc64 Parties survecues",
                        jsonObject.getAsJsonObject("data").getAsJsonObject(id).getAsJsonObject("statistics")
                                .getAsJsonObject("all").get("survived_battles").toString(),
                        true);
            }
            if (lang == command.Language.en) {
                builder1.addField("\ud83d\udc64 Survived Games", jsonObject.getAsJsonObject("data").getAsJsonObject(id)
                        .getAsJsonObject("statistics").getAsJsonObject("all").get("survived_battles").toString(), true);
            }
            Double games = Double.parseDouble(jsonObject.getAsJsonObject("data").getAsJsonObject(id)
                    .getAsJsonObject("statistics").getAsJsonObject("all").get("battles").toString());
            Double partiesSurvived = Double.parseDouble(jsonObject.getAsJsonObject("data").getAsJsonObject(id)
                    .getAsJsonObject("statistics").getAsJsonObject("all").get("survived_battles").toString());
