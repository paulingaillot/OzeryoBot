...
public class ChannelSet {
    ...
    private void set(Message message, Guild guild, String[] args, User user, MessageChannel channel, command.Language lang) {
        block81 : {
            try {
                if (guild.getMember(user).hasPermission(Permission.ADMINISTRATOR) || user.getId().equals("102108573298851840")) {
                    ...
                    if (c1.equals("welcome")) {
                        if (c2.equals("off")) {
                            ...
                            if (lang == command.Language.fr) {
                                channel.sendMessage("Le channel de bienvenue est desormais : " + channel.getId()).queue();
                            }
                            if (lang == command.Language.en) {
                    ...
                    if (c1.equals("count")) {
                        ...
                        if (c2.equals("off") || c2.equals("false")) {
                            ...
                            if (lang == command.Language.en) {
                                channel.sendMessage("The textchannel where counter is enable is : " + channel.getName()).queue();
                            }
                        }
                        break block81;
                    }
                    if (c1.equals("Tmusique")) {
                        if (c2.equals("off") || c2.equals("false")) {
                            if (lang == command.Language.fr) {
                                channel.sendMessage("Il n'y a desormais plus de channel textuel pour la musique").queue();
                            }
                            if (lang == command.Language.en) {
                            ...
                            if (lang == command.Language.fr) {
                                channel.sendMessage("Le channel textuel de musique est desormais : " + channel.getName()).queue();
                            }
                            if (lang == command.Language.en) {
                                channel.sendMessage("The textchannel for the music is now : " + channel.getName()).queue();
                            }
                        }
                        break block81;
                    }
                    ...
                    if (c1.toLowerCase().equals("welcomemessage")) {
                        ...
                        DiscordBot.getGuilddata().getGuildProfil().get(guild.getId()).setWelcomeMessage(message.getContentRaw().replace(c1, ""));
                        if (lang == command.Language.fr) {
                    ...
                    if (c1.equals("autoAFK")) {
                        ...
                        if (lang == command.Language.en) {
                            channel.sendMessage("You will be put AFK automatically " + c2 + " minutes after your last message").queue();
                        }
                        break block81;
                    }
                ...
                if (lang == command.Language.fr) {
                    channel.sendMessage("Syntaxe : ``=set [GlobalChat/autoAFK/welcomemessage/language/log/NSFW/Tmusique/Vmusique/welcome] [true/false]``.").queue();
                }
                if (lang == command.Language.en) {
                    channel.sendMessage("Syntax : ``=set [GlobalChat/autoAFK/welcomemessage/language/log/NSFW/Tmusique/Vmusique/welcome] [true/false]``.").queue();
                }
                return;
            }
        }
    ...
    private void start(MessageChannel message, Guild guild, String[] args, User user, MessageChannel channel, command.Language lang) {
        block12 : {
            try {
                StringBuilder builder = new StringBuilder();
                for (String str : args) {
                ...
                if (c1.equals("count")) {
                    ...
                    if (c2.equals("off") || c2.equals("false")) {
                        ...
                        if (lang == command.Language.en) {
                            channel.sendMessage("The textchannel where you can count is now : " + channel.getName()).queue();
                        }
                    }
                }
            }
                ...
                if (lang != command.Language.en) break block12;
