...
public class Pub {
    ...
    private void pub(Message message, Guild guild, String[] args, User user, TextChannel channel, String arg, JDA jda, ProfilData data, command.Language lang) {
        if (guild.getMember(user).hasPermission(Permission.ADMINISTRATOR)) {
            ...
            if (c1.equals("")) {
                ...
                channel.sendMessage("Votre nouveau channel de pub est : **" + channel.getName() + "**. Pour enregistrer la pub de votre guild utiliser la commande ``=pub set [ta pub]``").queue();
            } else if (c1.equals("set")) {
                if (c2.contains("@everyone") || c2.contains("@here") || message.getMentionedUsers().size() > 0 || message.getMentionedChannels().size() > 0) {
