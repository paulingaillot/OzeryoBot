...
public class reputation {
    ...
    private void rep(MessageChannel channel, User user, String[] args, Message message, command.Language lang) {
        ...
        if (c1.equals("")) {
            if (delay < 86400000L) {
                ...
                if (lang == command.Language.fr) {
                    channel.sendMessage("Vous avez " + rep + " de Réputation. Vous pourrez en donner un dans "
                            + (23 - mHour) + " heures " + (60 - mMinute) + " minutes " + (60 - mSecond) + " secondes.")
                            .queue();
                }
                if (lang == command.Language.en) {
                    channel.sendMessage("You have " + rep
                            + " reputation points. You will be able to give a reputation point in " + (23 - mHour)
                            + " hours " + (59 - mMinute) + " minutes " + (59 - mSecond) + " seconds.").queue();
                }
            } else {
                if (lang == command.Language.fr) {
                    channel.sendMessage(
                            "Vous avez " + rep + " de Réputation. Vous pouvez donner un point de reputation").queue();
                }
                if (lang == command.Language.en) {
                    channel.sendMessage("You have " + rep
                            + " reputation points. You can give a reputation point to another player.").queue();
                }
            }
            return;
        }
        if (delay >= 86400000L) {
            ...
            if (lang == command.Language.fr) {
                channel.sendMessage("Vous venez de donnez un point de reputation a " + cible.getAsMention() + ".")
                        .queue();
            }
            if (lang == command.Language.en) {
            ...
            if (lang == command.Language.fr) {
                channel.sendMessage(
                        "Vous ne pouvez pas encore donner un point de reputation. Votre Prochain point de reputation sera disponible  dans"
                                + (23 - mHour) + " heures " + (60 - mMinute) + " minutes " + (60 - mSecond)
                                + " secondes. Vous avez " + rep + "de reputation")
                        .queue();
            }
            if (lang == command.Language.en) {
                channel.sendMessage("You can't give a reputation point yet. Your reputation point will be available in "
                        + (23 - mHour) + " hours " + (60 - mMinute) + " minutes " + (60 - mSecond)
                        + " seconds. You have " + rep + " reputation points").queue();
            }
        }
    }
}
