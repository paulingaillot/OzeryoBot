...
public class Spy {
    ...
    private void spy(Message message, Guild guild, String[] args, User user, MessageChannel channel, String arg,
        ...
        if (caserne <= 25) {
            if (lang == command.Language.fr) {
                channel.sendMessage(
                        "Vous devez ameliorer votre camp d'entrainement au niveau 26 afin de debloquer les espions.")
                        .queue();
            }
            if (lang == command.Language.en) {
                channel.sendMessage("You must upgrade your  camp d'entrainement  to level 26 to unlock spying.")
                        .queue();
            }
            return;
        }
        int Lday = Integer.parseInt(
                TextFileWriter.read("/home/DiscordBot/Rasberry/données/Users/" + user.getId() + "/spyday.txt"));
        GregorianCalendar calendar = new GregorianCalendar();
        int day = calendar.get(5);
        if (day != Lday) {
            point = (caserne - 25) * 2;
            TextFileWriter.write("/home/DiscordBot/Rasberry/données/Users/" + user.getId() + "/spy.txt",
                    Integer.toString(point), 1);
            TextFileWriter.write("/home/DiscordBot/Rasberry/données/Users/" + user.getId() + "/spyday.txt",
                    Integer.toString(day), 1);
        }
        if ((point = Integer.parseInt(
                TextFileWriter.read("/home/DiscordBot/Rasberry/données/Users/" + user.getId() + "/spy.txt"))) == 0) {
            if (lang == command.Language.fr) {
        ...
        if (lang == command.Language.fr) {
            ((UserImpl) user).getPrivateChannel()
                    .sendMessage("\ud83d\udd75 Rapport d'espionnage \ud83d\udd75 \n Vous venez d'espionnez la ville de "
                            + cible.getName() + ".\n Elle poss\u00e8de entre " + soldierMin + " et " + soldierMax
                            + " soldats." + messages)
                    .queue();
        }
        if (lang == command.Language.en) {
            ((UserImpl) user).getPrivateChannel()
                    .sendMessage("\ud83d\udd75 Spying report \ud83d\udd75 \n You jsut spy " + cible.getName()
                            + " city.\n there is between " + soldierMin + " and " + soldierMax
                            + " soldiers in this city. " + messages)
                    .queue();
        }
        TextFileWriter.write("/home/DiscordBot/Rasberry/données/Users/" + user.getId() + "/spy.txt",
                Integer.toString(point - 1), 1);
        if (lang == command.Language.fr) {
            channel.sendMessage("Votre mission d'espionnage est en cours... Consulter vos mp pour voir le rapport.")
                    .queue();
        }
        if (lang == command.Language.en) {
